...
export function createLanguageModel(config: AgentProviderConfig): LanguageModel {
  ...
  if (providerKind === 'anthropic') {
    return createAnthropic({ apiKey, baseURL, name, headers })(model as any)
  }
  if (providerKind === 'google') {
    return createGoogleGenerativeAI({ apiKey, baseURL, name, headers })(model as any)
  }
  if (providerKind === 'openai-responses') {
    return createOpenAI({ apiKey, baseURL, name, headers }).responses(model as any)
  }

  return createOpenAICompatible({ name, apiKey, baseURL, headers, includeUsage: true }).chatModel(model)
}
